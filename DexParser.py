...
class Dexfile:
    ...
    def init_header(self):
        ...
        f.seek(0x24, 0)
        self.dexHeader.header_size = int(bytes.fromhex(binascii.b2a_hex(f.read(4)[::-1]).hex()).decode('utf-8'),16)

        f.seek(0x28, 0)
        self.dexHeader.endian_tag = bytes.fromhex(binascii.b2a_hex(f.read(4)[::-1]).hex()).decode('utf-8')

        f.seek(0x2c, 0)
        self.dexHeader.link_size = int(bytes.fromhex(binascii.b2a_hex(f.read(4)[::-1]).hex()).decode('utf-8'),16)

        f.seek(0x30, 0)
        self.dexHeader.link_off = int(bytes.fromhex(binascii.b2a_hex(f.read(4)[::-1]).hex()).decode('utf-8'),16)

        f.seek(0x34, 0)
        self.dexHeader.map_off =int(bytes.fromhex(binascii.b2a_hex(f.read(4)[::-1]).hex()).decode('utf-8'),16)

        f.seek(0x38, 0)
        self.dexHeader.string_ids_size = int(bytes.fromhex(binascii.b2a_hex(f.read(4)[::-1]).hex()).decode('utf-8'),16)

        f.seek(0x3c, 0)
        self.dexHeader.string_ids_off = int(bytes.fromhex(binascii.b2a_hex(f.read(4)[::-1]).hex()).decode('utf-8'),16)

        f.seek(0x40, 0)
        self.dexHeader.type_ids_size = int(bytes.fromhex(binascii.b2a_hex(f.read(4)[::-1]).hex()).decode('utf-8'),16)

        f.seek(0x44, 0)
        self.dexHeader.type_ids_off = int(bytes.fromhex(binascii.b2a_hex(f.read(4)[::-1]).hex()).decode('utf-8'),16)

        f.seek(0x48, 0)
        self.dexHeader.proto_ids_size = int(bytes.fromhex(binascii.b2a_hex(f.read(4)[::-1]).hex()).decode('utf-8'),16)
        
        f.seek(0x4c, 0)
        self.dexHeader.proto_ids_off = int(bytes.fromhex(binascii.b2a_hex(f.read(4)[::-1]).hex()).decode('utf-8'),16)

        f.seek(0x50, 0)
        self.dexHeader.field_ids_size = int(bytes.fromhex(binascii.b2a_hex(f.read(4)[::-1]).hex()).decode('utf-8'),16)

        f.seek(0x54, 0)
        self.dexHeader.field_ids_off = int(bytes.fromhex(binascii.b2a_hex(f.read(4)[::-1]).hex()).decode('utf-8'),16)

        f.seek(0x58, 0)
        self.dexHeader.method_ids_size = int(bytes.fromhex(binascii.b2a_hex(f.read(4)[::-1]).hex()).decode('utf-8'),16)

        f.seek(0x5c, 0)
        self.dexHeader.method_ids_off = int(bytes.fromhex(binascii.b2a_hex(f.read(4)[::-1]).hex()).decode('utf-8'),16)

        f.seek(0x60, 0)
        self.dexHeader.class_defs_size = int(bytes.fromhex(binascii.b2a_hex(f.read(4)[::-1]).hex()).decode('utf-8'),16)

        f.seek(0x64, 0)
        self.dexHeader.class_defs_off = int(bytes.fromhex(binascii.b2a_hex(f.read(4)[::-1]).hex()).decode('utf-8'),16)

        f.seek(0x68, 0)
        self.dexHeader.data_size = int(bytes.fromhex(binascii.b2a_hex(f.read(4)[::-1]).hex()).decode('utf-8'),16)

        f.seek(0x6c, 0)
        self.dexHeader.data_off = int(bytes.fromhex(binascii.b2a_hex(f.read(4)[::-1]).hex()).decode('utf-8'),16)
    ...
    def init_class_defs(self):
        ...
        for i in range(class_ids_size):
            ...
            for i in range(direct_method_size):
                ...
                self.dexClassData.virtualMethods.append(dexMethod)
                offset += length
            
    def parseDexCode(self, codeOff):
        self.dexHeader.f.seek(codeOff, 0)
        registers_size = binascii.b2a_hex()
